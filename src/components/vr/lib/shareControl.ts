...
import { offMatterportUi } from './offMatterportUi';
...
export function shareControl(mpsdk: MpSdk, model: VrModel, viewer: Element) {
  ...
  const listToggle = function (mutationsList: MutationRecord[]) {
    for (const mutation of mutationsList) {
      if (mutation.target instanceof Element && mutation.target.childElementCount === 1) {
        ...
        if (modalBody && input && input.length >= 1) {
          input[0].remove();
  ...
  const bottomObserve = () => offMatterportUi(viewer);
